# Přesný postup nahrání AI Studio GHRAB 0.20.2 na GitHub
...
1. Nejprve nasaďte aktuální chráněné verze dílčích aplikací:
   ...
   - Korespondenční asistent 5.9.1,
   - LUDUS 1.16.3,
   - ACTIVA 0.5.0,
   - SORTIO 1.0.2,
   - Lesson Hub 1.2.0.
2. U Lesson Hubu nejprve ověřte živý manifest `/lesson-hub/studio-manifest.json` a interaktivní manuál `/lesson-hub/manual/`.
3. Potom nahrajte obsah balíku AI Studio GHRAB 0.20.2 do kořene repozitáře `AI-Studio-GHRAB`.
...
V patičce Studia a v `dist/build-info.json` musí být verze **0.20.2**.

## Pořadí pro Core

1. Nejprve nasaďte KS 5.9.1 s `aiCore` manifestem a `ai-operations.json`.
2. Potom nasaďte AI Studio 0.20.2; jeho workflow načte živý manifest KS během synchronizace.
