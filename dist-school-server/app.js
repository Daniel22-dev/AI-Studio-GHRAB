import { sanitizePilotEvent, sanitizePilotEventList } from "./privacy/pilot-event.js?v=0.21.67";
import { validateMaterialPackage } from "./shared/material-validator.js?v=0.21.67";
import { buildPilotSummary } from "./shared/safe-export.js?v=0.21.67";
import {
  applyDeploymentToAppRegistry,
  loadDeploymentConfig,
} from "./access/deployment-config.js?v=0.21.67";
import { initialisePlatformRuntime } from "./access/platform-runtime.js?v=0.21.67";
import { createRegistryClient } from "./modules/registry-client.js?v=0.21.67";
import {
  initialiseAccess,
  setPermitToken,
  clearPermit,
  ...
  formatReason,
  inspectPermitToken,
} from "./access/access-control.js?v=0.21.67";
const VERSION = "0.21.67";
...
function academyLaunchUrl(deployment = null) {
  const fallback = new URL("/AI-Akademie-GHRAB/", location.href);
  const academyBase = deployment?.appBaseUrls?.["ai-academy"] || fallback.href;
  const target = new URL(academyBase, location.href);
  if (["http:", "https:"].includes(location.protocol) && target.origin !== location.origin) {
    return fallback.href;
  }
  target.searchParams.set("from", "ai-studio");
  target.searchParams.set(
    "studio",
    deployment?.studioBaseUrl || new URL(base, location.href).href,
  );
  return target.href;
}

function ensureAcademyNavigation() {
  const nav = document.querySelector(".main-nav");
  ...
  link.dataset.en = "AI Academy";
  link.dataset.csAria = "Otevřít školicí středisko AI Akademie";
  link.dataset.enAria = "Open the AI Academy training centre";
  link.textContent = t("AI Akademie", "AI Academy");
  link.setAttribute(
    "aria-label",
    t("Otevřít školicí středisko AI Akademie", "Open the AI Academy training centre"),
  );
  link.hidden = true;
  const colleaguePreview = nav.querySelector('[data-nav="colleague-preview"]');
  nav.insertBefore(link, colleaguePreview || null);
  void deploymentReady
    .then((deployment) => {
      link.href = academyLaunchUrl(deployment);
    })
    .catch((error) =>
      console.warn("AI Studio: adresu AI Akademie se nepodařilo načíst z deployment konfigurace.", error),
    );
}
...
async function setupHeaderLivePresence() {
  ...
  try {
    const { mountHeaderLivePresence } = await import("./modules/header-live-presence.js?v=0.21.67");
...
async function loadAppTestStatusModule() {
  ...
  appTestStatusModule ||= await import("./modules/app-test-status.js?v=0.21.67");
  return appTestStatusModule;
}
async function loadOperationalStatusModule() {
  operationalStatusModule ||= await import("./modules/operational-status.js?v=0.21.67");
...
applyMotion();
renderHome();
void import('./modules/portal-effects.js?v=0.21.67')
...
void Promise.all([deploymentReady, import("./access/app-guard.js?v=0.21.67")])
