import { safeExportSelfTest } from "../shared/safe-export.js?v=0.21.50";
...
if (T.canAccessAdminPage?.("tests")) {
  ...
  function card(label, status, detail) {
    ...
    article.className = `test-card ${status}`;
    const mark = document.createElement("span");
    mark.textContent = status === "ok" ? "✓" : status === "pending" ? "…" : "!";
    const h = document.createElement("h2");
    h.textContent = label;
    const p = document.createElement("p");
    p.textContent = detail;
    article.append(mark, h, p);
    return article;
  }
  async function imageOk(src) {
    return new Promise((resolve) => {
      ...
      image.src = `${src}${src.includes("?") ? "&" : "?"}qa=${Date.now()}`;
    });
  }
  ...
  function storageCheck() {
    const key = "ghrab.selftest";
    const value = `ok-${Date.now()}`;
    const written = T.safeSetItem(key, value, { silent: true });
    const read = T.safeGetItem(key) === value;
    T.safeRemoveItem(key);
    return written && read;
  }
  function validHandoff() {
    const material = {
      schema: "ghrab-material-v1",
      id: "selftest-material",
      version: 1,
      title: "Selftest",
      subject: "QA",
      yearGroup: "",
      level: "",
      language: "cs",
      objectives: ["Ověřit přenos"],
      content: {
        sourceText: "Testovací obsah bez osobních údajů.",
        vocabulary: [],
        tasks: [],
      },
    ...
    const created = T.createHandoff("generator", material);
    const ok =
      Boolean(created) && T.readHandoff()?.material?.id === material.id;
    T.clearHandoff();
    return ok;
  }
  function renderSummary(results) {
    ...
    a.className = "chip";
    a.textContent = T.t(
      `${passed} kontrol v pořádku`,
      `${passed} checks passed`,
    );
    const b = document.createElement("span");
    b.className = "chip";
    b.textContent = failed
      ? T.t(`${failed} problémů`, `${failed} issues`)
      : T.t("Bez zjištěné chyby", "No detected error");
    summary.append(a, b);
  }
  async function runChecks() {
    ...
    box.replaceChildren(
      card(
        ...
        "pending",
        T.t(
          "Načítám konfiguraci a ověřuji lokální funkce.",
          "Loading configuration and verifying local functions.",
        ),
      ),
    );
    const results = [];
    try {
      ...
      results.push({
        ...
        detail: T.t(
          `${apps.length} aplikace načteny.`,
          `${apps.length} applications loaded.`,
        ),
      });
      ...
      results.push({
        ...
        detail: T.t(
          `${icons.filter(Boolean).length}/${icons.length} ikon je dostupných.`,
          `${icons.filter(Boolean).length}/${icons.length} icons are available.`,
        ),
      });
    } catch (error) {
      results.push({
    ...
    results.push({
      ...
      status: access.valid && T.isAdmin() ? "ok" : "fail",
      detail: access.valid
        ? T.t(
            `Platný přístup: ${access.permit.jti}.`,
            `Valid permit: ${access.permit.jti}.`,
          )
        : T.formatReason(access.reason, T.state.language),
    });
    try {
      const [policy, key, revocations] = await Promise.all(
        [
      ...
      results.push({
        ...
        status: ok ? "ok" : "fail",
        detail: T.t(
          `${Object.keys(policy.applications || {}).length} aplikace · klíč ${key.keyId}.`,
          `${Object.keys(policy.applications || {}).length} applications · key ${key.keyId}.`,
        ),
      });
    } catch (error) {
      results.push({
    ...
    results.push({
      ...
      detail: storage
        ? T.t(
            "Bezpečné čtení, zápis a odstranění fungují.",
            "Safe read, write and removal work.",
          )
        : T.t(
            "Prohlížeč blokuje místní úložiště.",
    ...
    try {
      results.push({
        ...
        detail: T.t(
          "Krátkodobý přenos platného GHRAB Material v1 funguje.",
          "Short-term transfer of a valid GHRAB Material v1 package works.",
        ),
      });
    } catch (error) {
      results.push({
    ...
    results.push({
      ...
      detail: T.t(
        `Neplatný testovací soubor zachytil ${invalid.errors.length} problémů.`,
    ...
    results.push({
      ...
      detail: T.t(
        "Skutečné exportní transformace nepropustily záměrně vložená citlivá data.",
        "Real export transformations did not leak intentionally inserted sensitive data.",
      ),
    });
    const pages = [
      "../",
    ...
    results.push({
      ...
      detail: T.t(
        `${pageChecks.filter(Boolean).length}/${pageChecks.length} stránek odpovědělo úspěšně.`,
        `${pageChecks.filter(Boolean).length}/${pageChecks.length} pages responded successfully.`,
      ),
    });
    ...
    results.push({
      ...
      detail: T.t(
        `${guardChecks.filter(Boolean).length}/${guardChecks.length} integračních souborů je dostupných.`,
        `${guardChecks.filter(Boolean).length}/${guardChecks.length} integration files are available.`,
      ),
    });
    ...
    results.push({
      ...
      detail: worker
        ? T.t(
            "Prohlížeč podporuje service worker.",
            "The browser supports service workers.",
          )
        : T.t(
            "Tento prohlížeč service worker nepodporuje.",
    ...
    results.push({
      ...
      detail: T.t(
        `Studio používá přibližně ${usage.kilobytes} kB místního úložiště.`,
        `The Studio uses approximately ${usage.kilobytes} kB of local storage.`,
      ),
    });
    box.replaceChildren(
