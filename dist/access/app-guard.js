...
const OUTPUT_KINDS = Object.freeze({
  ...
  correspondence: new Set(['incoming-analysis', 'reply-draft', 'outgoing-email']),
...
function writeJson(key, value){
  ...
  catch (error) { console.warn(`AI Studio telemetry: zápis selhal pro ${key}`, error); return false; }
}
function removeStorage(key){ try { localStorage.removeItem(key); } catch { /* best effort */ } }
function eventId(){ return `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`; }
function quantity(value){ return Math.max(0, Math.min(10000, Math.round(Number(value || 0)))); }
function modeForSnapshot(snapshot){
  const requested = localStorage.getItem(TELEMETRY_MODE_KEY) === 'test' ? 'test' : 'live';
  return snapshot?.permit?.role === 'admin' && requested === 'test' ? 'test' : 'live';
}
...
function recordOutput(appId, keys, payload = {}){
  ...
  if (!allowedKinds.has(outputKind)) {
    console.warn(`AI Studio telemetry: nepovolený typ výstupu ${appId}/${outputKind}`);
    return false;
  }
  ...
  const suppliedOutcome = ['success', 'error', 'partial', 'cancelled'].includes(payload.outcome) ? payload.outcome : '';
  if (!attemptedQuantity) attemptedQuantity = successfulQuantity + failedQuantity + cancelledQuantity;
  ...
  list.push({
    ...
    failedQuantity,
    cancelledQuantity,
    outcome: suppliedOutcome || derivedOutcome(successfulQuantity, failedQuantity, cancelledQuantity)
  });
  return writeJson(keys.events, list.slice(-MAX_EVENTS));
}

function exposeTelemetry(appId, keys, mode){
  const existing = window.GHRABTelemetry && typeof window.GHRABTelemetry === 'object' ? window.GHRABTelemetry : {};
  const api = {
    ...existing,
    appId,
    mode,
    recordOutput: payload => recordOutput(appId, keys, payload),
    recordGeneration: outcome => recordOutput(appId, keys, {
      outputKind: appId === 'generator' ? 'test-package' : [...(OUTPUT_KINDS[appId] || [])][0],
      attemptedQuantity: 1,
...
function recordApplicationOpen(appId, keys){
  if (!appId || document.documentElement.dataset.ghrabLaunchRecorded === 'true') return;
  document.documentElement.dataset.ghrabLaunchRecorded = 'true';
  const launches = readJson(keys.launches, {});
  const safeLaunches = launches && typeof launches === 'object' && !Array.isArray(launches) ? launches : {};
  const current = safeLaunches[appId] && typeof safeLaunches[appId] === 'object' ? safeLaunches[appId] : {};
  const now = new Date();
  ...
  current.monthly = current.monthly && typeof current.monthly === 'object' && !Array.isArray(current.monthly) ? current.monthly : {};
  const bucket = current.monthly[period] && typeof current.monthly[period] === 'object' ? current.monthly[period] : {};
...
function startActiveTimeTracker(appId, keys){
  if (!appId || document.documentElement.dataset.ghrabActiveTracker === 'ready') return;
  ...
  const isEligible = () => document.visibilityState === 'visible' && document.hasFocus() && (Date.now() - lastInteractionAt) <= ACTIVE_IDLE_MS;
  const acquireLock = () => {
    const now = Date.now();
    const lock = readJson(lockKey, null);
    if (lock && lock.owner !== tabId && Number(lock.expiresAt || 0) > now) return false;
    if (!writeJson(lockKey, { owner: tabId, expiresAt: now + ACTIVE_LOCK_TTL_MS })) return false;
  ...
  const addActiveSeconds = seconds => {
    ...
    const timestamp = now.toISOString();
    const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    current.activeSeconds = Math.max(0, Number(current.activeSeconds || 0)) + seconds;
    current.activeSessions = Math.max(0, Number(current.activeSessions || 0)) + (creditedPeriods.has('all') ? 0 : 1);
    current.lastActiveAt = timestamp;
    current.monthly = current.monthly && typeof current.monthly === 'object' && !Array.isArray(current.monthly) ? current.monthly : {};
    const bucket = current.monthly[period] && typeof current.monthly[period] === 'object' ? current.monthly[period] : {};
    bucket.count = Math.max(0, Number(bucket.count || 0));
    bucket.lastOpened = bucket.lastOpened || null;
    bucket.activeSeconds = Math.max(0, Number(bucket.activeSeconds || 0)) + seconds;
    bucket.activeSessions = Math.max(0, Number(bucket.activeSessions || 0)) + (creditedPeriods.has(period) ? 0 : 1);
  ...
  const tick = () => {
    ...
    const elapsedMs = Math.max(0, Math.min(now - lastTickAt, ACTIVE_MAX_CREDIT_MS));
    lastTickAt = now;
    if (!isEligible()) { releaseLock(); return; }
    if (!acquireLock()) return;
    addActiveSeconds(Math.max(1, Math.round(elapsedMs / 1000)));
  };
  const resetTick = () => { lastTickAt = performance.now(); if (!isEligible()) releaseLock(); };
  ...
  window.addEventListener('focus', () => { markInteraction(); resetTick(); }, { passive: true });
  window.addEventListener('blur', resetTick, { passive: true });
  document.addEventListener('visibilitychange', resetTick, { passive: true });
  window.addEventListener('pagehide', () => { clearInterval(timer); releaseLock(); }, { once: true });
...
export async function protectApp(appId, options = {}){
  ...
  if (snapshot.ready && access.enabled) {
    ...
    if (options.telemetry !== false) {
      ...
      if (options.errorReporter !== false) setupErrorReporter({ appId, studioUrl: studioHref(options) });
    }
    document.dispatchEvent(new CustomEvent('ghrab:app-access-granted', { detail: { appId, permit: access.permit, telemetryMode: mode } }));
    return true;
  }
