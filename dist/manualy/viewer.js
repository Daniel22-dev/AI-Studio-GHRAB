...
function allowedManualUrl(raw){
  try {
    ...
    const allowedOrigins = new Set([location.origin, 'https://daniel22-dev.github.io']);
...
function updateHeader(){
  ...
  frame.title = G.t(`Interaktivní manuál: ${G.localised(currentApp.name)}`, `Interactive manual: ${G.localised(currentApp.name)}`);
}

function renderLocked(access){
  const training = G.requiredTraining(currentApp.id);
  const detail = access.reason === 'app-not-permitted' && training
    ? G.t(`Tento manuál se odemkne stejným oprávněním jako aplikace po školení ${training.trainingCode} · verze ${training.trainingVersion}.`, `This manual unlocks with the same application permit after training ${training.trainingCode} · version ${training.trainingVersion}.`)
    : G.formatReason(access.reason, G.state.language);
  showState('locked', G.t('Tento manuál je uzamčen', 'This manual is locked'), detail, [
    linkButton(G.t('Otevřít Můj přístup', 'Open My access'), '../access/', 'button primary'),
...
function loadFrame(){
  ...
  statePanel.hidden = false;
  frame.hidden = true;
  stateTitle.textContent = G.t('Načítám aktuální manuál…', 'Loading the current manual…');
  stateCopy.textContent = G.t('Manuál zůstane otevřený přímo uvnitř nainstalovaného AI Studia.', 'The manual will stay open directly inside the installed AI Studio.');
  stateActions.replaceChildren();
  frame.src = currentManualUrl.href;
  loadTimer = setTimeout(() => {
    if (frame.hidden) {
      stateCopy.textContent = G.t('Načítání trvá déle. Zkontrolujte připojení; případně použijte volbu Otevřít zvlášť.', 'Loading is taking longer. Check your connection or use Open separately.');
...
async function initialise(){
  ...
  try {
    ...
    if (!currentApp) {
      showState('error', G.t('Manuál nebyl nalezen', 'Manual not found'), G.t('Odkaz neodpovídá žádné aplikaci v aktuálním registru.', 'The link does not match any application in the current registry.'), [linkButton(G.t('Zpět na přehled', 'Back to catalogue'), './', 'button primary')]);
      return;
    }
    updateHeader();
    ...
    if (!currentManualUrl) {
      showState('error', G.t('Adresa manuálu není bezpečná', 'The manual address is not safe'), G.t('AI Studio odmítlo neplatnou nebo neočekávanou adresu manuálu.', 'AI Studio rejected an invalid or unexpected manual address.'), [linkButton(G.t('Zpět na přehled', 'Back to catalogue'), './', 'button primary')]);
      return;
    }
    ...
    showState('error', G.t('Manuál se nepodařilo připravit', 'The manual could not be prepared'), G.t('Obnovte stránku nebo se vraťte do katalogu manuálů.', 'Refresh the page or return to the manual catalogue.'), [linkButton(G.t('Zpět na přehled', 'Back to catalogue'), './', 'button primary')]);
  }
}
