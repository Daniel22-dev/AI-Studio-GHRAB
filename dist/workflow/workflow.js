...
function renderTargets(){
  ...
  targetHost.replaceChildren(...apps.map(app=>{
    ...
    const accessNote=document.createElement('small');accessNote.className=`target-access ${access.enabled?'ok':'locked'}`;accessNote.textContent=access.enabled?G.t('Přístup aktivní','Access active'):G.formatReason(access.reason,G.state.language);
    const button=document.createElement('button');button.type='button';button.className='button target-button';
    if(!access.enabled){button.textContent=G.t('Aktivovat přístup','Activate access');button.addEventListener('click',()=>{location.href='../access/'});}
    else{button.disabled=!state.ready||!fields.private.checked;button.textContent=state.ready?G.t('Připravit a otevřít','Prepare and open'):G.t('Chybí podklady','Missing inputs');button.addEventListener('click',()=>handoff(app));}
    item.append(head,accessNote,button);return item;
  }));
}
...
function handoff(app){
  const access=G.hasAppAccess(app.id);
  if(!access.enabled){G.showToast(G.t('Tato aplikace není ve vašem přístupu odemčena.','This application is not unlocked in your access.'));location.href='../access/';return;}
  const material=collectMaterial();
...
async function importFile(file){
  ...
  if(!fileCheck.valid){const error=fileCheck.errors[0];G.showToast(G.t(error.cs,error.en));return;}
  try{
    const material=JSON.parse(await file.text());
    const result=G.validateMaterialPackage(material);
    if(!result.valid){const error=result.errors[0];throw new Error(G.t(`${error.cs} (${error.path})`,`${error.en} (${error.path})`));}
    setMaterial(material);lastSavedSignature=materialSignature(material);G.showToast(G.t('Balíček byl bezpečně ověřen a načten.','The package was safely validated and loaded.'));
  }catch(error){G.showToast(error instanceof SyntaxError?G.t('Soubor neobsahuje platný JSON.','The file does not contain valid JSON.'):(error.message||G.t('Soubor není platný GHRAB Material v1.','The file is not a valid GHRAB Material v1 package.')));}
}
...
Promise.all([G.loadApps(),G.accessReady]).then(([loaded])=>{apps=loaded.filter(app=>app.id!=='correspondence');renderTargets();});
