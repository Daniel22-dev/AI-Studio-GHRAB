...
if(consumer.appId!=='ai-studio'){check('source.no-src-platform',!exists(path.join(root,'src','platform')));check('source.no-public-platform',!exists(path.join(root,'public','platform')));const vendorVersion=String(consumer.platform?.version||'').trim();check('source.vendor-platform-version',Boolean(vendorVersion),vendorVersion||'missing');check('source.vendor-platform',Boolean(vendorVersion)&&exists(path.join(root,'vendor',`ghrab-platform-${vendorVersion}`,'ghrab-platform.js')),vendorVersion||'missing')}
