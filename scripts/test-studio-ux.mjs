...
const academyBridgeEnd = appJs.indexOf("let headerLivePresenceMounted", academyBridgeStart);
const academyBridgeSource = academyBridgeStart >= 0 && academyBridgeEnd > academyBridgeStart
...
check(/import\(\s*[\"']\.\/modules\/app-test-status\.js[\"']\s*\)/.test(appJs) && appJs.includes('if (isAdmin() && !isColleaguePreview() && appTestStatusModule)') && appJs.includes('createAppTestStatusButton(app, article, state.language)'), "Stav testovani neni odlozene nacten a dostupny na kazde karte jen ve skutecnem spravcovskem pohledu.");
