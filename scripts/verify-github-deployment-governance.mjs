...
const response = await fetch(`https://api.github.com/repos/${repo}/branches/main`, {
  headers: {
...
if (required.enforcement_level !== 'everyone') {
  console.error(`GITHUB GOVERNANCE FAIL: required checks nejsou vynuceny i pro administratory (enforcement=${required.enforcement_level || 'unknown'})`);
  process.exit(1);
}

const configured = new Set([
  ...(Array.isArray(required.contexts) ? required.contexts : []),
  ...(Array.isArray(required.checks) ? required.checks.map((item) => item?.context).filter(Boolean) : []),
]);
const missing = REQUIRED_STATUS_CHECKS.filter((name) => !configured.has(name));
if (missing.length) {
  console.error(`GITHUB GOVERNANCE FAIL: chybi required check(s): ${missing.join(', ')}`);
  process.exit(1);
}
