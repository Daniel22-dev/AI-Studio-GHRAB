...
function emitChange() {
  ...
  document.dispatchEvent(
    new CustomEvent("ghrab:access-changed", { detail: getAccessSnapshot() }),
  );
}
...
function verificationKeys(publicKeyInfo) {
  if (Array.isArray(publicKeyInfo?.keys) && publicKeyInfo.keys.length) {
    return publicKeyInfo.keys;
  }
  if (
...
function verificationKeyFor(kid) {
  return (
    verificationKeys(accessState.publicKeyInfo).find(
      (item) => item?.keyId === kid || item?.publicKey?.kid === kid,
    ) || null
  );
}
function validateClaims(payload, policy, revocations) {
  if (!payload || payload.schema !== "ghrab-access-permit-v1")
    return "invalid-schema";
  if (payload.iss !== policy.issuer || payload.aud !== policy.audience)
    return "invalid-audience";
  if (!payload.sub || !payload.jti || !payload.kid || !payload.role)
    return "missing-claims";
  if (!Number.isFinite(payload.iat) || !Number.isFinite(payload.exp))
    return "invalid-time-claims";
  const skew = Number(policy.clockSkewSeconds || 300);
  const now = nowSeconds();
  if (Number.isFinite(payload.nbf) && payload.nbf > now + skew)
  ...
  const enforcementTime =
    Date.parse(policy.maximumPermitDaysEnforcedAfter || "") / 1000;
  const isLegacyPermit =
    Number.isFinite(enforcementTime) && payload.iat < enforcementTime;
  ...
  const maximum = maximumDays * 86400 + skew;
  if (payload.exp - payload.iat > maximum) return "validity-too-long";
  if (
    revocations?.revokedBefore &&
    payload.iat <= Math.floor(Date.parse(revocations.revokedBefore) / 1000)
  )
    return "revoked-by-date";
  if (
    Array.isArray(revocations?.revokedJti) &&
    revocations.revokedJti.includes(payload.jti)
  )
    return "revoked";
  return null;
}
  ...
  try {
    ...
    return valid
      ? {
          valid: true,
          reason: "valid",
          permit: { ...payload, apps: normaliseApps(payload.apps) },
        }
...
export async function initialiseAccess(options = {}) {
  ...
  try {
    const [policy, revocations, publicKeyInfo] = await Promise.all([
      fetchJson(
        options.policyUrl
          ? new URL(options.policyUrl, location.href)
          : new URL("access-policy.json", CONFIG_BASE),
      ),
      fetchJson(
        options.revocationsUrl
          ? new URL(options.revocationsUrl, location.href)
          : new URL("revoked-access.json", CONFIG_BASE),
      ),
      fetchJson(
        options.publicKeyUrl
          ? new URL(options.publicKeyUrl, location.href)
          : new URL("access-public-key.json", CONFIG_BASE),
      ),
    ]);
    if (policy.schema !== "ghrab-access-policy-v1")
      throw new Error("invalid policy schema");
    if (revocations.schema !== "ghrab-access-revocation-list-v1")
      throw new Error("invalid revocation schema");
    if (!verificationKeys(publicKeyInfo).length)
...
export async function setPermitToken(token) {
  ...
  const result = await verifyToken(clean);
  if (!result.valid)
    return { ok: false, reason: result.reason, permit: result.permit || null };
  if (!safeStorageSet(TOKEN_KEY, clean))
...
export function getAccessSnapshot() {
  return {
    ...
    permit: accessState.permit
      ? { ...accessState.permit, apps: [...(accessState.permit.apps || [])] }
      : null,
...
export function getPermitToken() {
  return accessState.valid ? accessState.token : null;
}
export function isAdmin() {
  if (!accessState.valid) return false;
  return (accessState.policy?.administratorRoles || ["admin"]).includes(
    accessState.permit?.role,
  );
}
function trainingFailure(appId) {
  ...
  if (
    held.code !== required.trainingCode ||
    held.version !== required.trainingVersion
  )
    return "training-outdated";
  return null;
}
export function hasAppAccess(appId) {
  if (!accessState.ready)
    return { enabled: false, reason: "loading", permit: null };
...
export function formatReason(reason, language = "cs") {
  const messages = {
    cs: {
      loading: "Ověřuji přístup…",
      missing: "Přístup zatím nebyl aktivován.",
      "invalid-format": "Přístupový kód má neplatný formát.",
      "invalid-payload": "Přístupový kód je poškozený.",
      "invalid-schema": "Přístupový kód používá nepodporovanou verzi.",
      "invalid-audience": "Přístupový kód nepatří k tomuto Studiu.",
      "missing-claims": "V přístupovém kódu chybí povinné údaje.",
      "invalid-time-claims": "Přístupový kód obsahuje neplatné datum.",
      "not-yet-valid": "Přístup ještě není platný.",
      "issued-in-future": "Přístupový kód má neplatné datum vydání.",
      expired: "Platnost přístupu skončila.",
      "invalid-validity-window": "Přístupový kód má neplatnou dobu platnosti.",
      "validity-too-long": "Doba platnosti přístupu překračuje povolený limit.",
      "revoked-by-date": "Tento přístup byl centrálně zneplatněn.",
      revoked: "Tento přístup byl správcem zneplatněn.",
      "unknown-key": "Přístupový kód byl podepsán neznámým klíčem.",
      "invalid-signature": "Digitální podpis přístupu není platný.",
      "verification-error": "Přístup se nepodařilo kryptograficky ověřit.",
      "configuration-unavailable": "Konfiguraci přístupů se nepodařilo načíst.",
      "app-not-permitted": "Tato aplikace není v přístupu odemčena.",
      "training-missing":
        "Přístup neobsahuje potvrzení požadovaného školení pro tuto aplikaci.",
      "training-outdated":
        "Pro tuto aplikaci je nutné obnovit školení a vydat nové oprávnění.",
      administrator: "Správcovský přístup je aktivní.",
      permitted: "Přístup k aplikaci je aktivní.",
      "storage-error": "Prohlížeč nepovolil uložení přístupu.",
      "invalid-file": "Soubor s přístupem nelze přečíst.",
      "file-too-large": "Soubor s přístupem je příliš velký.",
      "missing-file": "Nebyl vybrán soubor.",
    },
    en: {
      loading: "Verifying access…",
      missing: "Access has not been activated yet.",
      "invalid-format": "The access code has an invalid format.",
      "invalid-payload": "The access code is damaged.",
      "invalid-schema": "The access code uses an unsupported version.",
      "invalid-audience": "The access code does not belong to this Studio.",
      "missing-claims": "Required access data is missing.",
      "invalid-time-claims": "The access code contains an invalid date.",
      "not-yet-valid": "Access is not valid yet.",
      "issued-in-future": "The access code has an invalid issue date.",
      expired: "Access has expired.",
      "invalid-validity-window": "The access validity period is invalid.",
      "validity-too-long": "The access validity exceeds the allowed limit.",
      "revoked-by-date": "This access has been centrally revoked.",
      revoked: "This access has been revoked by the administrator.",
      "unknown-key": "The access code was signed by an unknown key.",
      "invalid-signature": "The digital signature is invalid.",
      "verification-error": "Access could not be cryptographically verified.",
      "configuration-unavailable":
        "The access configuration could not be loaded.",
      "app-not-permitted": "This application is not unlocked in your access.",
      "training-missing":
        "The permit does not contain the required training confirmation for this application.",
      "training-outdated":
        "Training must be renewed and a new permit issued for this application.",
      administrator: "Administrator access is active.",
      permitted: "Application access is active.",
      "storage-error": "The browser did not allow access to be saved.",
      "invalid-file": "The access file could not be read.",
      "file-too-large": "The access file is too large.",
