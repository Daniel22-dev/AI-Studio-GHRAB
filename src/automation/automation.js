...
if (window.GHRAB.isAdmin()) {
  ...
  function renderTelemetryMode() {
    ...
    if (telemetryStatus)
      telemetryStatus.textContent =
        mode === "test"
          ? t(
              "Testovací data jsou oddělená a nejsou v pilotním reportu.",
              "Test data are separated and excluded from the pilot report.",
            )
          : t(
  ...
  function renderTable(report, apps) {
    ...
    for (const source of report?.sources || []) {
      ...
      const values = [
        localised(app?.name) || source.id,
        source.url,
        source.ok
          ? t("Ověřeno", "Verified")
          : t("Použit záložní registr", "Fallback registry"),
        source.version || app?.version || "—",
      ];
  ...
  async function render() {
    ...
    summaryHost.replaceChildren(
      kpi(
        apps.length,
        t("aplikací", "applications"),
        t("aplikací", "applications"),
      ),
      kpi(`${ok}/${total}`, "manifestů ověřeno", "manifests verified"),
      kpi(report?.mode || "fallback", "režim synchronizace", "sync mode"),
      kpi(time, "poslední build", "latest build"),
    );
    renderTable(report || { sources: [] }, apps);
  }
  render().catch(() => {
    if (appHost)
      appHost.textContent = t(
        "Data automatizace se nepodařilo načíst.",
