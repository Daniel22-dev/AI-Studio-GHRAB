...
if (window.GHRAB.isAdmin()) {
  ...
  const buttons = [...document.querySelectorAll("[data-step]")],
    ...
    autoplay = document.querySelector("#demo-autoplay"),
    notesButton = document.querySelector("#demo-notes-toggle");
  function stopAuto() {
    if (timer) {
      clearInterval(timer);
      timer = null;
    }
    autoplay.textContent = window.GHRAB.t(
      "▶ Spustit automaticky",
      "▶ Start autoplay",
    );
  ...
  function show(next) {
    ...
    buttons.forEach((b) =>
      b.setAttribute("aria-current", b.dataset.step === id ? "step" : "false"),
    );
    ...
    nextBtn.textContent =
      index === steps.length - 1
        ? window.GHRAB.t("Hotovo", "Done")
        : window.GHRAB.t("Další →", "Next →");
    document
      .querySelector("#demo-progress-bar")
      .style.setProperty(
        "--demo-progress",
  ...
  buttons.forEach((b) =>
    b.addEventListener("click", () => {
      stopAuto();
      show(steps.indexOf(b.dataset.step));
    }),
  );
  ...
  autoplay.addEventListener("click", () => {
    ...
    autoplay.textContent = window.GHRAB.t("⏸ Pozastavit", "⏸ Pause");
    autoplay.setAttribute("aria-pressed", "true");
    timer = setInterval(() => show(index + 1), 12000);
  });
  notesButton.addEventListener("click", () => {
    notes = !notes;
    document.documentElement.dataset.speakerNotes = String(notes);
    notesButton.setAttribute("aria-pressed", String(notes));
  });
