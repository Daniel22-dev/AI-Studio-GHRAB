...
## Fail-closed chování při čekající předávce

`saveMaterialToStudio()` vrací `{ ok: false, reason: "handoff-pending" }`, pokud již existuje nevyzvednutá předávka. V takovém případě **nenaviguje** do AI Studia a nepřepisuje starší materiál. Volající má uživateli vysvětlit, že musí nejprve dokončit nebo vyzvednout předchozí předávku. Při skutečném selhání zápisu vrací `reason: "handoff-write-failed"`.
