...
function accessText(access, appId) {
  if (access.enabled)
    return access.reason === "administrator"
      ? G.t(
          "Správcovský přístup: manuál je odemčen.",
          "Administrator access: the manual is unlocked.",
        )
      : G.t(
          "Manuál je odemčen stejným oprávněním jako aplikace.",
          "The manual is unlocked by the same permit as the application.",
        );
  const training = G.requiredTraining(appId);
  if (access.reason === "app-not-permitted" && training) {
    return G.t(
      `Vyžaduje přístup po školení ${training.trainingCode} · verze ${training.trainingVersion}.`,
      `Requires access after training ${training.trainingCode} · version ${training.trainingVersion}.`,
    );
  }
...
function manualCard(app) {
  ...
  const badge = make(
    "span",
    `manual-access-badge ${enabled ? "open" : "locked"}`,
    enabled ? G.t("✓ Odemčeno", "✓ Unlocked") : G.t("🔒 Uzamčeno", "🔒 Locked"),
  );
  ...
  if (enabled) {
    ...
    const locked = make(
      "button",
      "button secondary manual-locked-button",
      G.t("Manuál uzamčen", "Manual locked"),
    );
    locked.type = "button";
    locked.disabled = true;
    actions.append(locked);
    const accessLink = make(
      "a",
      "button ghost",
      G.t("Můj přístup", "My access"),
    );
...
function renderSummary() {
  ...
  if (G.isAdmin()) {
    copy.append(
      make(
        ...
        "",
        G.t(
          "Správce: všechny manuály jsou dostupné",
          "Administrator: all manuals are available",
        ),
      ),
      make(
        "small",
        "",
        G.t(
          `Odemčeno ${apps.length} z ${apps.length} průvodců.`,
          `${apps.length} of ${apps.length} guides unlocked.`,
        ),
      ),
    );
    ...
    copy.append(
      ...
      make(
        ...
        "",
        G.t(
          `Odemčeno ${unlocked} z ${apps.length} manuálů podle absolvovaných školení.`,
          `${unlocked} of ${apps.length} manuals unlocked according to completed training.`,
        ),
      ),
    );
  } else {
    copy.append(
      ...
      make(
        ...
        "",
        G.t(
          "Karty zůstávají viditelné, jejich manuály však otevřete až po načtení platného oprávnění.",
          "Cards remain visible, but their manuals open only after a valid permit has been loaded.",
        ),
      ),
    );
  }
  const link = make(
    ...
    "button secondary",
    snapshot.valid
      ? G.t("Spravovat přístup", "Manage access")
      : G.t("Aktivovat přístup", "Activate access"),
  );
  link.href = "../access/";
  ...
  if (!apps.length) {
    grid.append(
      make(
        ...
        "manual-empty",
        G.t(
          "Registr manuálů se nepodařilo načíst.",
          "The manual registry could not be loaded.",
        ),
      ),
    );
    return;
...
function renderSyncState() {
  if (!apps.length) {
    syncState.textContent = G.t(
      "Registr se nepodařilo načíst. Obnovte stránku.",
      "The registry could not be loaded. Refresh the page.",
    );
  ...
  if (live === total && total > 0) {
    ...
    syncState.textContent = G.t(
      `K dispozici je bezpečný vestavěný registr ${apps.length} aplikací; živě ověřeno ${live}/${total}.`,
      `A safe built-in registry of ${apps.length} applications is available; ${live}/${total} verified live.`,
    );
    syncState.className = "manual-sync-state warn";
  }
}
