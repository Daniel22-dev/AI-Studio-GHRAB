const SAFE_EVENT_FIELDS = [
  'at', 'type', 'appId', 'outputKind', 'outcome',
  'attemptedQuantity', 'successfulQuantity', 'failedQuantity', 'cancelledQuantity'
];

const LEGACY_GENERATION_KIND = 'test-package';

function quantity(value){ return Math.max(0, Math.min(10000, Math.round(Number(value || 0)))); }
...
export function safeEvent(event = {}) {
  ...
  if (source.type === 'generation') {
    ...
    source.successfulQuantity = source.successfulQuantity ?? (source.outcome === 'success' ? 1 : 0);
    source.failedQuantity = source.failedQuantity ?? (source.outcome === 'error' ? 1 : 0);
    source.cancelledQuantity = source.cancelledQuantity ?? (source.outcome === 'cancelled' ? 1 : 0);
  }
  const output = {};
  ...
  if (output.type === 'output' || output.type === 'generation') {
    ...
    const accounted = output.successfulQuantity + output.failedQuantity + output.cancelledQuantity;
    if (!output.attemptedQuantity) output.attemptedQuantity = accounted || 1;
    if (accounted > output.attemptedQuantity) output.attemptedQuantity = accounted;
    if (!['success', 'error', 'partial', 'cancelled'].includes(output.outcome)) {
      output.outcome = output.failedQuantity && output.successfulQuantity ? 'partial'
        : output.failedQuantity ? 'error'
          : output.successfulQuantity ? 'success' : 'cancelled';
    }
  }
  return output;
}
...
export function safeLaunches(launches = {}) {
  const output = {};
  for (const [id, item] of Object.entries(launches || {})) output[id] = safeLaunchItem(item);
  return output;
}
...
export function safeLaunchesForPeriod(launches = {}, period = periodOfDate()) {
  ...
  for (const [id, item] of Object.entries(launches || {})) {
    const bucket = item?.monthly && typeof item.monthly === 'object' ? item.monthly[period] : null;
...
export function filterEventsByPeriod(events = [], period = periodOfDate()) {
  return (events || []).filter(event => periodOfDate(event?.at) === period).map(safeEvent);
}

export function filterWorkspaceByPeriod(workspace = [], period = periodOfDate()) {
  return (workspace || []).filter(material => {
    const timestamp = material?.provenance?.updatedAt || material?.provenance?.createdAt || material?.updatedAt || material?.createdAt;
    return periodOfDate(timestamp) === period;
  });
}

function emptyOutputBucket(){
  return { attempted: 0, successful: 0, failed: 0, cancelled: 0, events: 0, kinds: {} };
}

export function aggregateEvents(events = []) {
  const eventCounts = {};
  const appEventCounts = {};
  const outputs = { attempted: 0, successful: 0, failed: 0, cancelled: 0, events: 0, byApp: {} };
  ...
  for (const rawEvent of events || []) {
    ...
    app.events += 1;
    const kind = app.kinds[outputKind] || { attempted: 0, successful: 0, failed: 0, cancelled: 0, events: 0 };
...
export function safeStatistics({ launches = {}, events = [], workspace = [] } = {}) {
  ...
  const activeSeconds = Object.values(launchCounts).reduce((sum, item) => sum + Number(item.activeSeconds || 0), 0);
  ...
  const completedOutputs = aggregates.outputs.successful + aggregates.outputs.failed;
  ...
  return {
    ...
    outputSuccessRate: completedOutputs ? aggregates.outputs.successful / completedOutputs : 0,
    ...
    generationSuccessRate: (generator.successful + generator.failed) ? generator.successful / (generator.successful + generator.failed) : 0,
    qualityCounts,
    launchCounts,
...
export function buildPilotSummary({ portalVersion, currentWeek, currentPhase, sourceId = '', period = periodOfDate(), launches = {}, events = [], workspace = [] } = {}) {
  ...
  const statistics = safeStatistics({ launches: periodLaunches, events: periodEvents, workspace: periodWorkspace });
  return {
    ...
    methodology: {
      ...
      activeTime: 'Počítá se jen viditelná a aktivní karta se zaměřeným oknem. Po 5 minutách bez interakce se měření zastaví; dlouhé prodlevy systému se nezapočítávají.',
      outputs: 'Výstupy se zapisují přímo v jednotlivých aplikacích jako anonymní technické součty úspěšných, chybných a zrušených položek. Neobsahují jejich obsah.',
      sourceId: 'Náhodný měsíční identifikátor prohlížeče slouží pouze k nahrazení opakovaně odeslaného souhrnu ze stejného zařízení a měsíce; neobsahuje identitu uživatele.'
    },
    privacy: 'Bez jmen, e-mailů, studentských dat, promptů, názvů materiálů, obsahu materiálů, klávesových vstupů a volných poznámek.'
  };
}

export function buildImpactReport({ portalVersion, settings = {}, launches = {}, events = [], workspace = [], caseCount = 0, externalMaterialCount = 0 } = {}) {
...
export function safeExportSelfTest() {
  ...
  const report = buildPilotSummary({
    ...
    period,
    launches: { generator: { count: 9, activeSeconds: 999, monthly: { [period]: { count: 1, lastOpened: null, activeSeconds: 45, activeSessions: 1 } } } },
    events: [poisoned],
    workspace: [{ title: 'TAJNY_MATERIAL', content: { sourceText: 'TAJNY_TEXT' }, provenance: { updatedAt: new Date().toISOString() } }]
  });
  ...
  return !leakedPoison && !unsafeEventKey && report.schema === 'ghrab-pilot-summary-v8-safe' && report.launches.generator.activeSeconds === 45 && report.launches.generator.count === 1 && report.totals.outputSuccess === 1;
}
